    ...
    if (isset($this->projects[$item->id])) {
        foreach ($this->projects[$item->id] as $project) {?>
           <?php if($project['thumb']) {?> 
            ...
            if(!$this->modal) { ?>
            	<a href="<?php echo $this->params->get("images_directory") . "/".$project['image'];?>" >
            <?php }else{
                
                if("slimbox" == $this->modal ) { // Slimbox
                    echo '<a href="'.  $this->params->get("images_directory") . "/".$project['image'].'" rel="lightbox-item'.$item->id.'" >';
                }else { // Native
                    echo '<a href="'.  $this->params->get("images_directory") . "/".$project['image'].'" class="vip-modal" rel="{handler: \'image\'}" >';
                }
            }
            ?>
            <img src="<?php echo $this->params->get("images_directory") . "/".$project['thumb'];?>" alt="<?php echo $this->escape($project['title']);?>"
